...
function TimeAnalysisPage() {
  ...
  const weekAnalysis = useMemo(() => {
    ...
    dayAnalysis.forEach((day) => {
      ...
      if (day.workingMinutes > 0) {
        ...
        if (
          !analysis[weekKey].earliestStart ||
          (day.workStart &&
            day.workStart.isBefore(analysis[weekKey].earliestStart!))
        ) {
          analysis[weekKey].earliestStart = day.workStart;
        }
        if (
          !analysis[weekKey].latestEnd ||
          (day.workEnd && day.workEnd.isAfter(analysis[weekKey].latestEnd!))
        ) {
    ...
    Object.values(analysis).forEach((week) => {
      week.avgWorkDayHours =
        week.workDays > 0 ? week.workingMinutes / 60 / week.workDays : 0;
    });
  ...
  const overallStats = useMemo(() => {
    const totalWorking = dayAnalysis.reduce(
      (sum, day) => sum + day.workingMinutes,
      0,
    );
    const totalFree = dayAnalysis.reduce(
      (sum, day) => sum + day.freeMinutes,
      0,
    );
    ...
    return {
      ...
      avgStartTime: dayjs()
        .startOf("day")
        .add(Math.round(avgStartMinutes), "minute"),
      avgEndTime: dayjs()
        .startOf("day")
        .add(Math.round(avgEndMinutes), "minute"),
    };
  }, [dayAnalysis]);
  ...
  useEffect(() => {
    if (periodType === "month" && !selectedMonth && monthOptions.length > 0) {
      ...
      setSelectedMonth(
        currentMonthOption ? currentMonthOption.value : monthOptions[0].value,
      );
    }
  ...
  const monthChartData = useMemo(() => {
    ...
    if (selectedMonth) {
      ...
      dayAnalysis
        .filter(
          (day) =>
            (day.date.isAfter(monthStart) ||
              day.date.isSame(monthStart, "day")) &&
    ...
    return filteredDays.map((day, index) => {
      ...
      const isWeekStart =
        prevWeekNumber !== null && weekNumber !== prevWeekNumber;
      const year = day.date.year();
      const prevYear = prevDay ? prevDay.date.year() : null;
      // Учитываем смену года
      const isWeekStartWithYear =
        prevYear !== null &&
        (year !== prevYear || weekNumber !== prevWeekNumber);
  ...
  const hourlyDistribution = useMemo(() => {
    ...
    if (periodType === "week" && selectedWeek) {
      const [year, week] = selectedWeek
        .split("-W")
        .map((part, idx) => (idx === 0 ? parseInt(part) : parseInt(part)));
      ...
      filteredEntries = allEntries.filter((entry) => {
        ...
        const entryDate = dayjs(entry.begin);
        return (
          (entryDate.isAfter(monthStart) ||
            entryDate.isSame(monthStart, "day")) &&
          ...
          <Stack gap="md">
                ...
                referenceLines={[
                  ...
                  ...(() => {
                    ...
                    if (prevWasWeekend && monthChartData.length > 0) {
                      const lastDate =
                        monthChartData[monthChartData.length - 1].date;
            ...
            <Group gap="xs" mt="xs">
              <Badge size="sm" color="red" variant="light">
                Выходные (красная подсветка)
              </Badge>
              <Badge size="sm" color="blue" variant="light">
                Начало недели (вертикальная линия)
              </Badge>
            </Group>
          </Stack>
        </Paper>
        ...
        <Paper p="md" withBorder>
          <Stack gap="md">
            ...
            <BarChart
              ...
              dataKey="label"
              series={[
                { name: "hours", color: "blue.6", label: "Часы работы" },
              ]}
            />
            <Text size="xs" c="dimmed" ta="center">
              Период:{" "}
              {periodType === "week" && selectedWeek
                  ...
                  {viewMode === "day"
                    ...
                    : weekAnalysis.slice(0, 30).map((item, index) => (
                        <Table.Tr key={index}>
                          ...
                          <Table.Td>
                            <Badge variant="light">{item.workDays} дней</Badge>
                          </Table.Td>
                          <Table.Td>
                            <Text>
                              {formatDuration(item.avgWorkDayHours * 60)}
                            </Text>
                          </Table.Td>
                        </Table.Tr>
                      ))}
