...
function CalendarPage() {
  ...
  const { weeks, loading, error, syncing } = useDashboardData(
    settings,
    syncStatus,
  );
  ...
  function clampEventToDay(entry: Timesheet, day: dayjs.Dayjs) {
    ...
    const entryEnd = rawEnd.isValid()
      ? rawEnd
      : entryStart.add(MIN_EVENT_MINUTES, "minute");
  ...
  function addGapsToSegments(
    segments: DaySegment[],
  ): Array<
    DaySegment & { top: number; height: number; left: number; width: number }
  > {
    ...
    sortedSegments.forEach((segment) => {
      ...
      for (const column of columns) {
        ...
        const hasOverlap = column.segments.some(
          ({ endTime, segment: colSegment }) => {
            ...
            return (
              segmentStartTime < colEndTime && segmentEndTime > colStartTime
            );
          },
        );
  ...
  useEffect(() => {
    if (view === "week" || view === "day") {
      ...
      const scrollPosition =
        ...
        currentMinute * MINUTE_HEIGHT -
        200; // Отступ сверху 200px

      setTimeout(() => {
        if (view === "week" && weekScrollRef.current) {
          const scrollArea = weekScrollRef.current.querySelector(
        ...
        if (view === "day" && dayScrollRef.current) {
          const scrollArea = dayScrollRef.current.querySelector(
            ".mantine-ScrollArea-viewport",
          ) as HTMLElement;
          if (scrollArea) {
            scrollArea.scrollTop = Math.max(0, scrollPosition);
  ...
  const currentDaySegments = useMemo(
    () => getSegmentsForDay(dayjs(currentDate), allEntries),
                  ...
                  {(() => {
                    ...
                    return days.map((day, index) => {
                      ...
                      const isCurrentMonth =
                        dayjs(day).month() === dayjs(currentDate).month();
                              ...
                              {dayEntries.slice(0, 3).map((entry) => {
                                ...
                                return (
                                  <Badge
                                    ...
                                    style={{ fontSize: "10px" }}
                                  >
                                    {dayjs(entry.begin).format("HH:mm")}{" "}
                            ...
                            <Tooltip
                              ...
                              label={
                                <Stack gap={4}>
                                  ...
                                  <Text size="xs">
                                    Время: {begin.format("HH:mm")} -{" "}
                                    {isRunning ? "..." : end.format("HH:mm")}
                                  </Text>
                                  <Text size="xs">
          ...
          <Paper p="md" withBorder>
            <Group justify="space-between" mb="md">
              ...
              <Group gap="xs">
                ...
                <Badge color="green" variant="light" size="lg">
                  {formatDuration(
                    weekEntries.reduce((sum, e) => sum + (e.duration || 0), 0),
                  )}
                </Badge>
              </Group>
            </Group>
            ...
            <ScrollArea h={770} ref={weekScrollRef}>
              <Box style={{ display: "flex", gap: "12px" }}>
                <Box
                  style={{ width: "80px", flexShrink: 0, position: "relative" }}
                >
                ...
                >
                  ...
                  >
                    {Array.from({ length: 7 }).map((_, dayIndex) => {
                      ...
                      const daySegments = getSegmentsForDay(
                        dayDate,
                        allEntries,
                      );
                      const dayTotalMinutes = daySegments.reduce(
                      ...
                      return (
                        ...
                        >
                          <Stack gap="xs" mb="xs">
                            ...
                            <Badge
                              size="xs"
                              color={dayTotalMinutes > 0 ? "green" : "gray"}
                              variant="light"
                            >
                          ...
                          >
                            ...
                            {Array.from({ length: 24 }).map((_, hour) => (
                              <Box
                                ...
                                style={{
                                  ...
                                  height: "1px",
                                  borderTop:
                                    "1px dashed var(--mantine-color-dark-5)",
                                  zIndex: 1,
                                }}
                              />
                            ))}
                            ...
                            {(() => {
                              const now = dayjs(currentTime);
                              if (!isToday || !now.isSame(dayDate, "day"))
                                return null;
                              const currentHour = now.hour();
                              const currentMinute = now.minute();
                              const top =
                      ...
                      <Tooltip
                        ...
                        label={
                          <Stack gap={4}>
                            ...
                            {/* <Text size="xs">{activityName}</Text> */}
                            <Text size="xs">
                              Время: {segmentStart.format("HH:mm")} -{" "}
                              {isRunning ? "..." : segmentEnd.format("HH:mm")}
                            </Text>
                            <Text size="xs">
                        ...
                        <Card
                          ...
                          style={{
                            ...
                            borderLeft: isRunning
                              ...
                              : `4px solid var(--mantine-color-${color}-7)`,
                            cursor: "pointer",
        ...
        <Paper p="md" withBorder>
          <Stack gap="xs">
            ...
            <DatePicker
              value={currentDate}
              onChange={(value) =>
                handleDateChange(value as Date | string | null)
              }
              locale="ru"
              numberOfColumns={3}
