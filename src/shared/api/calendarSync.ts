...
export class GoogleCalendarSync {
  ...
  async authorize(): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    localStorage.setItem('google-client-secret', this.settings.googleClientSecret)
  ...
  async refreshAccessToken(): Promise<string> {
    if (!this.settings.googleRefreshToken || !this.settings.googleClientId || !this.settings.googleClientSecret) {
   ...
   */
  async syncEvents(entries: Timesheet[]): Promise<{ created: number; updated: number; errors: number }> {
    ...
    for (const entry of entries) {
      try {
        ...
        await this.createOrUpdateEvent(calendarId, accessToken, event, entry.id)
        created++
      } catch (error) {
  ...
  private async createOrUpdateEvent(
    ...
    if (updateResponse.status === 404) {
      ...
      if (!createResponse.ok) {
        throw new Error(`Ошибка создания события: ${createResponse.statusText}`)
      }
    } else if (!updateResponse.ok) {
      throw new Error(`Ошибка обновления события: ${updateResponse.statusText}`)
    }
  }
}
...
async function notionFetch(url: string, options: RequestInit): Promise<Response> {
  ...
  if (electronAvailable && window.electron?.notionApi) {
    ...
    console.warn('Notion API: Using browser fetch (CORS may fail). Electron IPC not available.')
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(`База данных не найдена (HTTP 404). Проверьте Database ID: ${this.settings.notionDatabaseId}. Убедитесь, что интеграция имеет доступ к базе данных.`)
        }
        console.warn('Ошибка поиска страницы в Notion:', response.status, response.statusText)
        return null
      }
      ...
      if (error instanceof Error && error.message.includes('База данных не найдена')) {
        throw error
      }
