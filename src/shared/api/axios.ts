...
export const instance = axios.create({
    baseURL: typeof __DOMAIN_BACKEND__ !== 'undefined' ? (__DOMAIN_BACKEND__ as any) : '',
    headers: {
    ...
    inst.interceptors.response.use(
        ...
        async (error) => {
            ...
            if ((status === 401 || status === 403) && !originalRequest._retry) {
                ...
                if (!refreshToken) {
                    try { logoutEvents.emit() } catch (e) { console.error(e) }
                ...
                try {
                    if (!isRefreshing) {
                        isRefreshing = true
                        refreshPromise = inst.post('/api/auth/refresh', { refresh_token: refreshToken })
                            .then((res) => {
                                const data = res.data
                                ...
                                if (newAccess) {
                                    ...
                                    try { setToken({ accessToken: newAccess, refreshToken: newRefresh }) } catch (e) {}
                                }
                                return data
                            })
                            .finally(() => {
                    ...
                    try { logoutEvents.emit() } catch (er) { console.error(er) }
...
export function createInstance(baseURL?: string) {
    ...
    if (authorizationToken) {
        inst.defaults.headers.common['Authorization'] = `Bearer ${authorizationToken}`
    }
