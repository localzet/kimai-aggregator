...
export function groupByWeek(
  ...
  timesheets.forEach((entry) => {
    ...
    grouped[weekKey].entries.push({
      ...
      isExcluded:
        entry.tags?.some((tag) =>
          excludedTagsLower.includes(tag.toLowerCase()),
        ) ?? false,
    });
  ...
  return weeks.map((week) => {
    ...
    week.entries.forEach((entry) => {
      const isExcluded =
        entry.isExcluded ??
        entry.tags?.some((tag) =>
          excludedTagsLower.includes(String(tag).toLowerCase()),
        ) ??
        false;
      if (!isExcluded) {
        ...
        projectStats[projectKey].amount =
          projectStats[projectKey].minutes * (isFinite(rate) ? rate : 0);
        ...
        if (projectId && projectSettings[projectId]) {
          ...
          if (
            settings.enabled &&
            settings.hasPaymentPeriods &&
            settings.paymentPeriodWeeks
          ) {
            ...
            if (weekYear === startYear) {
              ...
              weeksSinceStart =
                weeksInStartYear + weeksInBetweenYears + weekNumber;
            } else {
            ...
            if (weeksSinceStart >= 0) {
              const periodNumber = Math.floor(
                weeksSinceStart / settings.paymentPeriodWeeks,
              );
              const weekInPeriod =
                weeksSinceStart % settings.paymentPeriodWeeks;
              ...
              if (!projectPeriodInfo[projectId]) {
                projectPeriodInfo[projectId] = {
                  ...
                  minutes: 0,
                  hours: 0,
                  goalHours: settings.hasWeeklyGoal
              ...
              projectPeriodInfo[projectId].hours =
                projectPeriodInfo[projectId].minutes / 60;
              projectPeriodInfo[projectId].weeklyAmount =
                projectPeriodInfo[projectId].minutes *
