...
class Database {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORES.TIMESHEETS)) {
          const timesheetsStore = db.createObjectStore(STORES.TIMESHEETS, {
            keyPath: "id",
          });
        ...
        if (
          oldVersion < 2 &&
          !db.objectStoreNames.contains(STORES.PROCESSED_WEEKS)
        ) {
          const weeksStore = db.createObjectStore(STORES.PROCESSED_WEEKS, {
            keyPath: "weekKey",
          });
  ...
  async getMetadata(key: string): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () =>
        resolve(
          (request.result as { value: unknown } | undefined)?.value || null,
        );
      request.onerror = () => reject(request.error);
    });
  }
            ...
            const serializedWeek = {
              ...
              endDate: week.endDate
                ? dayjs.isDayjs(week.endDate)
                  ? week.endDate.toISOString()
                  : week.endDate
                : undefined,
              // Сериализуем Dayjs в entries
              entries:
                (week.entries as StoredEntry[] | undefined)?.map((entry) => ({
                  ...entry,
                  date: entry.date
                    ? dayjs.isDayjs(entry.date)
      ...
      request.onsuccess = () => {
        ...
        const restoredWeeks = weeks.map((w) => {
          ...
          return {
            ...
            entries:
              (entries as StoredEntry[] | undefined)?.map((entry) => ({
                ...entry,
                date: entry.date
