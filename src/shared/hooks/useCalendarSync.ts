...
export function useCalendarSync() {
  ...
  const sync = async (
    ...
    if (settings.syncType === "google") {
      if (!settings.googleAccessToken && !settings.googleRefreshToken) {
        notifications.show({
          title: "Ошибка",
          message:
            "Google Calendar не авторизован. Перейдите в Настройки и авторизуйте календарь.",
          color: "red",
        });
        return null;
      }
      ...
      if (!settings.notionApiKey || !settings.notionDatabaseId) {
        notifications.show({
          title: "Ошибка",
          message:
            "Notion API Key или Database ID не заполнены. Проверьте настройки.",
          color: "red",
        });
        return null;
      }
      ...
      if (!isElectron) {
        ...
        notifications.show({
          ...
          message:
            "Синхронизация с Notion может не работать в браузере из-за CORS. Убедитесь, что используете Electron версию.",
          color: "yellow",
          autoClose: 5000,
        });
      }
    ...
    try {
      ...
      console.log(
        `Синхронизация ${filteredEntries.length} записей с ${settings.syncType}`,
      );
