...
function processData(
  ...
  const enrichedTimesheets = timesheetsData.map((entry) => {
    const projectId =
      typeof entry.project === "number"
        ? entry.project.toString()
        : (entry.project as Project)?.id?.toString();
    const activityId =
      typeof entry.activity === "number"
        ? entry.activity.toString()
        : (entry.activity as Activity)?.id?.toString();
    return {
      ...entry,
      project: projectId
...
export function useDashboardData(
  ...
  const loadFromCache = useCallback(async (useProcessedCache = true) => {
    try {
      ...
      const [cachedTimesheets, cachedProjects, cachedActivities] =
        await Promise.all([
          db.getTimesheets(),
          db.getProjects(),
          db.getActivities(),
  ...
  useEffect(() => {
    ...
    const rateChanged =
      prevSettings === null || prevSettings.ratePerMinute !== currentRate;
    const projectSettingsChanged =
      prevSettings === null ||
      JSON.stringify(prevSettings.projectSettings) !==
        JSON.stringify(currentProjectSettings);
    const excludedTagsChanged =
      prevSettings === null ||
      JSON.stringify(prevSettings.excludedTags?.sort()) !==
        JSON.stringify(currentExcludedTags?.sort());

    if (
      weeks.length > 0 &&
      (rateChanged || projectSettingsChanged || excludedTagsChanged)
    ) {
