...
export function useUnifiedSync() {
  ...
  const syncNotion = useCallback(async (entries: Timesheet[]): Promise<{ created: number; updated: number; errors: number } | null> => {
    ...
    if (!settings.calendarSync.notionApiKey || !settings.calendarSync.notionDatabaseId) {
      return null
    }
    ...
    try {
      ...
      const result = await syncCalendar(filteredEntries, settings.calendarSync)
      return result
    } catch (error) {
  ...
  const performSync = useCallback(async (triggerSource: 'manual' | 'page-change' | 'periodic' = 'manual') => {
    if (syncing) {
      return
    }
