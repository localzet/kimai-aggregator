...
const ML_API_URL =
  (import.meta.env.VITE_ML_API_URL as string) || "https://kimai-ml.zorin.cloud";
...
export class MLClient {
  ...
  preprocessData(weeks: WeekData[], settings: Settings): MLInputData {
    ...
    for (const week of weeks) {
      for (const entry of week.entries || []) {
        ...
        const activity =
          typeof entry.activity === "object" ? entry.activity : null;
    ...
    return {
      ...
      settings: {
        ...
        project_settings: Object.entries(settings.projectSettings || {}).reduce(
          (acc, [key, value]) => {
            acc[parseInt(key)] = {
  ...
  async predict(
    weeks: WeekData[],
    settings: Settings,
  ): Promise<MLOutputData["forecasting"]> {
  ...
  async detectAnomalies(
    weeks: WeekData[],
    settings: Settings,
  ): Promise<MLOutputData["anomalies"]> {
    const input = this.preprocessData(weeks, settings);
  ...
  async getRecommendations(
    weeks: WeekData[],
    settings: Settings,
  ): Promise<MLOutputData["recommendations"]> {
    const input = this.preprocessData(weeks, settings);
  ...
  async analyzeProductivity(
    weeks: WeekData[],
    settings: Settings,
  ): Promise<MLOutputData["productivity"]> {
    const input = this.preprocessData(weeks, settings);
