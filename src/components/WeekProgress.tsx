      ...
      <Paper p="xl" withBorder>
        ...
        <Stack gap="md">
          <Group justify="space-between">
            ...
            <Text size="sm" c="dimmed">
              {(dayjs.isDayjs(week.startDate)
                ? week.startDate
                : dayjs(week.startDate)
              ).format("DD.MM")}{" "}
              -{" "}
              {(dayjs.isDayjs(week.endDate)
                ? week.endDate
                : dayjs(week.endDate)
              ).format("DD.MM")}
            </Text>
            ...
            <Card withBorder p="md">
              <Stack gap="sm">
                <Group justify="space-between">
                  ...
                  <Text fw={700} size="lg">
                    {formatDuration(week.rawTotalMinutes ?? week.totalMinutes)}
                  </Text>
                </Group>
                <Group justify="space-between">
                  <Text fw={500}>Оплачиваемые часы:</Text>
                  <Text fw={700} size="lg">
                    {week.totalHours?.toFixed(2)} ч
                  </Text>
                </Group>
                <Group justify="space-between">
                ...
                {(() => {
                  const excludedCount = week.entries
                    ? week.entries.filter((e) => e.isExcluded).length
                    : 0;
                  if (excludedCount > 0) {
                    return (
                      <Group justify="space-between">
                        <Text c="dimmed">
                          Исключено из расчёта: {excludedCount} записей
                        </Text>
                        <Text c="dimmed">
                          (они видны в таблице, но не учитываются для финсов и
                          прогресса)
                        </Text>
                      </Group>
                    );
              ...
              {week.projectPeriodInfo.map((projectInfo, index) => {
                const progressPercent =
                  projectInfo.goalHours !== null
                    ? Math.min(
                        100,
                        (projectInfo.hours / projectInfo.goalHours) * 100,
                      )
                    : null;
                return (
                  <MotionCard
                      ...
                      <Stack gap="sm">
                        ...
                        {projectInfo.goalHours !== null && (
                          <>
                            ...
                            <Progress
                              ...
                              color={
                                progressPercent && progressPercent >= 100
                                  ? "green"
                                  : progressPercent && progressPercent >= 75
                                    ? "yellow"
                                    : "red"
                              }
                              size="lg"
                            ...
                            projectInfo.remainingHours !== undefined &&
                            projectInfo.remainingHours > 0 ? (
                              <Group justify="space-between" mt="sm">
                                ...
                                <Badge color="red" size="lg">
                                  {formatDuration(
                                    Math.round(projectInfo.remainingHours * 60),
                                  )}
                                </Badge>
                              </Group>
                            ) : projectInfo.overGoal !== null &&
                              projectInfo.overGoal !== undefined &&
                              projectInfo.overGoal > 0 ? (
                              <Group justify="space-between" mt="sm">
                                <Text c="green" fw={500}>
                                  Перевыполнение:
                                </Text>
                                <Badge color="green" size="lg">
                                  +
                                  {formatDuration(
                                    Math.round(projectInfo.overGoal * 60),
                                  )}
                                </Badge>
                              </Group>
                            ) : null}
                        ...
                        <Group justify="space-between" mt="sm">
                          ...
                          <Text fw={500} size="lg">
                            {formatCurrency(projectInfo.weeklyAmount)}
                          </Text>
                        </Group>
                      </Stack>
                    </Card>
