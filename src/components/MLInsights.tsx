...
export default function MLInsights() {
  ...
  const { weeks, loading: dataLoading } = useDashboardData(
    settings,
    syncStatus,
  );

  const { forecast, loading: forecastLoading } = useForecasting(
    weeks,
    settings,
  );
  ...
  const { recommendations, loading: recommendationsLoading } =
    useRecommendations(weeks, settings);
  const { productivity, loading: productivityLoading } = useProductivity(
    weeks,
    settings,
  );
      ...
      <Tabs defaultValue="forecast">
        ...
        <Tabs.Panel value="forecast" pt="md">
          {forecastLoading ? (
            ...
            <Stack gap="md">
              ...
              {Object.keys(forecast.weekly_hours_by_project || {}).length >
                0 && (
                <Card withBorder p="md">
                  <Text fw={600} mb="xs">
                    Прогноз по проектам
                  ...
                  <Stack gap="xs">
                    {Object.entries(forecast.weekly_hours_by_project || {}).map(
                      ([projectId, hours]) => (
          ...
          {anomaliesLoading ? (
            ...
            <Stack gap="sm">
              {anomalies.map((anomaly) => (
                ...
                >
                  <Text size="sm" mb="xs">
                    <strong>Тип:</strong> {anomaly.type} |{" "}
                    <strong>Серьезность:</strong> {anomaly.severity}
                  </Text>
        ...
        <Tabs.Panel value="recommendations" pt="md">
          {recommendationsLoading ? (
            ...
            <Alert color="blue">
              Рекомендации будут доступны при наличии достаточного количества
              данных
            </Alert>
          )}
        </Tabs.Panel>
          ...
          {productivityLoading ? (
            ...
            <Stack gap="md">
              <Card withBorder p="md">
                ...
                <Text size="sm" mb="xs">
                  <strong>Время:</strong>{" "}
                  {productivity.optimal_work_hours.start}:00 -{" "}
                  {productivity.optimal_work_hours.end}:00
                </Text>
                <Text size="sm">
                  <strong>Дни:</strong>{" "}
                  {productivity.optimal_work_hours.days
                    ...
                    .map((e) => (
                      <Group key={e.hour} justify="space-between">
                        ...
                        <Group gap="xs" style={{ flex: 1, maxWidth: 200 }}>
                          <Progress
                            value={e.efficiency * 100}
                            size="sm"
                            style={{ flex: 1 }}
                          />
                          <Text size="xs" c="dimmed">
                            {(e.efficiency * 100).toFixed(0)}%
                          </Text>
                        </Group>
                      </Group>
                    ))}
              ...
              <Card withBorder p="md">
                ...
                <Text size="sm">
                  ...
                  <strong>
                    {productivity.break_recommendations.optimal_break_duration}{" "}
                    минут
                  </strong>
                </Text>
                <Text size="sm" mt="xs">
                  Частота перерывов: каждые{" "}
                  <strong>
                    {productivity.break_recommendations.break_frequency} часа
                  </strong>
                </Text>
              </Card>
            </Stack>
            ...
            <Alert color="blue">
              Анализ продуктивности будет доступен при наличии достаточного
              количества данных
            </Alert>
          )}
        </Tabs.Panel>
