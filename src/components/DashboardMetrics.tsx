...
const DashboardMetrics = memo(function DashboardMetrics({
  ...
  const stats = useMemo(() => {
    ...
    const currentWeek = weeks.find(
      (w) => w.year === now.year() && w.week === now.isoWeek(),
    );
      ...
      <Grid>
        ...
        <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
          <Card withBorder p="lg" radius="md">
            ...
            <Text fw={700} size="lg">
              {stats.avgHoursPerWeek.toFixed(1)} ч
            </Text>
            <Text size="sm" c="dimmed">
              {formatCurrency(stats.avgAmountPerWeek)}
            </Text>
          </Card>
        </Grid.Col>
        ...
        <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
          <Card withBorder p="lg" radius="md">
            ...
            <Text size="sm" c="dimmed">
              {stats.totalHours.toFixed(0)} часов
            </Text>
          </Card>
        </Grid.Col>
      ...
      <Grid>
        <Grid.Col span={{ base: 12, md: 6 }}>
          <Paper p="md" withBorder>
            <Title order={4} mb="md">
              Тренд доходов (последние 12 недель)
            </Title>
          ...
          <Paper p="md" withBorder>
            ...
            {projectChartData.length > 0 ? (
              <BarChart
                ...
                dataKey="name"
                series={[{ name: "Доход (руб)", color: "green" }]}
              />
            ) : (
              <Text c="dimmed" ta="center" py="lg">
                Недостаточно данных для графика
        ...
        <Grid>
          <Grid.Col span={{ base: 12, sm: 6 }}>
            <Group justify="space-between" mb="xs">
              ...
              <Text fw={700} size="sm">
                {formatCurrency(forecast.yearlyForecast)}
              </Text>
            </Group>
            <Text size="xs" c="dimmed">
              При сохранении текущего среднего заработка{" "}
              {formatCurrency(stats.avgAmountPerWeek)} в неделю
            </Text>
          </Grid.Col>
          ...
          <Grid.Col span={{ base: 12, sm: 6 }}>
            <Group justify="space-between" mb="xs">
              ...
              <Text fw={700} size="sm">
                {stats.weeksCount} недель данных
              </Text>
            </Group>
            <Text size="xs" c="dimmed">
              Средний доход за час:{" "}
              {formatCurrency(stats.totalAmount / (stats.totalHours || 1))}/ч
            </Text>
          </Grid.Col>
        </Grid>
      </Paper>
